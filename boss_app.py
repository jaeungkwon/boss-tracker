...
@st.cache_data(ttl=60) # 60초(1분)마다 데이터를 새로 읽어옵니다.
...
if not df_live.empty:
    # 1행 3열에 있는 '업데이트 시간'을 가져옵니다. (에러 방지를 위해 컬럼 인덱스 사용)
    update_time = df_live.iloc[0, 2] if len(df_live.columns) > 2 else "알 수 없음"
    
    # 만약 '업데이트 시간'이라는 글씨가 데이터로 인식되었다면, 그 다음 줄의 실제 시간을 가져옵니다.
    if update_time == "업데이트 시간" and len(df_live) > 1:
        update_time = df_live.iloc[1, 2]
        df_live = df_live.iloc[1:].reset_index(drop=True)

    st.caption(f"⏱️ 마지막 서버 확인 시간: {update_time} (1분마다 자동 갱신)")
    ...
    if "생존한 보스가 없습니다" in str(df_live.iloc[0, 0]):
        ...
        # Key Error 방지: 이름으로 찾지 않고 무조건 첫 번째, 두 번째 열을 가져와서 이름을 덮어씌웁니다.
        display_df = df_live.iloc[:, :2].copy()
        display_df.columns = ["생존 보스", "출몰 맵"]
